const state = { config: null, step: 0, service: null, barber: null, date: new Date().toISOString().slice(0, 10), time: null, loading: false, error: null };
...
async function confirmBooking() {
  ...
  try {
    const response = await fetch("/api/public/bookings", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ tenant: state.config.slug || state.config.id, serviceId: state.service.id, barberId: state.barber.id, startAt: state.time, customer: { name: data.get("name"), phone: data.get("phone") } }) });
    const result = await response.json();
...
function render() {
  ...
  if (state.step === 4) {
    app.innerHTML = `<div class="shell success"><div class="success-mark">✓</div><h1>Agendamento confirmado</h1><p class="meta">${state.service.name} com ${state.barber.name}<br>${formatDateTime(state.time)}</p><button class="ghost" id="again">Novo agendamento</button></div>`;
  ...
  app.innerHTML = `<div class="shell"><header class="brand"><strong>${state.config.name}</strong><span class="badge">AGENDA ONLINE</span></header>${state.error ? `<div class="alert">${state.error}</div>` : ""}${pages[state.step]()}</div><div class="bottom"><div class="bottom-inner"><div class="step">Etapa ${state.step + 1} de 4</div>${state.step > 0 ? `<div class="summary">${summary()}</div>` : ""}<button class="cta" id="next" ${canNext() && !state.loading ? "" : "disabled"}>${state.loading ? "Carregando…" : state.step === 3 ? "Confirmar agendamento" : "Continuar"}</button></div></div>`;
  bind();
}

function servicesPage() { return `<section class="hero"><h1>Seu próximo corte começa aqui.</h1><p>Escolha o serviço e encontre um horário realmente disponível.</p></section><section class="section"><h2>Serviços</h2><div class="grid">${state.config.services.map((service) => `<button class="card service" data-id="${service.id}" data-selected="${state.service?.id === service.id}"><div class="row"><div><strong>${service.name}</strong><div class="meta">${duration(service)} min</div></div><span class="price">${money(priceCents(service))}</span></div></button>`).join("")}</div></section>`; }
function barbersPage() { return `<section class="hero"><h1>Escolha seu barbeiro.</h1><p>Veja especialidades antes de reservar.</p></section><section class="section"><div class="grid">${state.config.barbers.map((barber) => `<button class="card barber" data-id="${barber.id}" data-selected="${state.barber?.id === barber.id}"><div class="row"><div><strong>${barber.name}</strong><div>${(barber.specialties || []).map((x) => `<span class="pill">${x}</span>`).join("")}</div></div><span>›</span></div></button>`).join("")}</div></section>`; }
...
function confirmPage() { return `<section class="hero"><h1>Confirme em segundos.</h1><p>Seus dados ficam vinculados somente a esta barbearia.</p></section><section class="section"><div class="card details"><div class="row"><span class="meta">Serviço</span><strong>${state.service.name}</strong></div><div class="row"><span class="meta">Barbeiro</span><strong>${state.barber.name}</strong></div><div class="row"><span class="meta">Horário</span><strong>${formatDateTime(state.time)}</strong></div><div class="row"><span class="meta">Valor</span><strong class="price">${money(priceCents(state.service))}</strong></div></div><form id="customer-form" class="form"><label>Nome<input name="name" autocomplete="name" required maxlength="80"></label><label>WhatsApp<input name="phone" inputmode="tel" autocomplete="tel" required minlength="10" maxlength="20"></label></form></section>`; }
...
function summary() { return [state.service && `<strong>${state.service.name}</strong>`, state.barber?.name, state.time && state.time.slice(11, 16)].filter(Boolean).join(" • "); }
function canNext() { return [!!state.service, !!state.barber, !!state.time, true][state.step]; }

function bind() {
  document.querySelectorAll(".service").forEach((element) => element.onclick = () => { state.service = state.config.services.find((x) => x.id === element.dataset.id); render(); });
